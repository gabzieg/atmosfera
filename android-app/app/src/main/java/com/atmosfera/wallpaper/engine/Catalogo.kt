package com.atmosfera.wallpaper.engine

/**
 * CONTRATO (congelado) — catálogo de cenários (wallpapers) do Atmosfera.
 *
 * Esta é a fonte da verdade dos wallpapers disponíveis. O FRONT lê isto para
 * montar a loja / o seletor de cenário. O MOTOR (nosso lado) atualiza a lista
 * conforme novos cenários ficam prontos — o front NÃO edita este arquivo.
 *
 * Convenções:
 *  - [id] = nome da pasta de assets em `assets/atmosfera/cenas/<id>/`
 *           (o cenário grátis "cabana" mora na raiz `assets/atmosfera/`).
 *  - [gratis] = já incluído (não precisa comprar).
 *  - [productId] = SKU de COMPRA AVULSA no Google Play (null se grátis).
 *                  Quem tem Premium destrava os efeitos "vivos" em todos.
 */
data class Cenario(
    val id: String,
    val nome: String,
    val descricao: String,
    val gratis: Boolean,
    val productId: String?,
)

object Catalogo {
    val cenarios: List<Cenario> = listOf(
        Cenario(
            id = "cabana",
            nome = "Cabana na floresta",
            descricao = "Refúgio de madeira à beira do lago, sob a mata.",
            gratis = true,
            productId = null,
        ),
        Cenario(
            id = "tanque",
            nome = "Tanque — campo de batalha",
            descricao = "Diorama em massinha de um Sherman no front de guerra.",
            gratis = false,
            productId = "cenario_tanque",
        ),
        Cenario(
            id = "fiordes",
            nome = "Fiordes — vila norueguesa",
            descricao = "Vila à beira de um fiorde, com barcos e farol.",
            gratis = false,
            productId = "cenario_fiordes",
        ),
        Cenario(
            id = "cabana2",
            nome = "Ruína na selva",
            descricao = "Ruína de pedra tomada pela selva, à beira de um lago.",
            gratis = false,
            productId = "cenario_cabana2",
        ),
        Cenario(
            id = "velhooeste",
            nome = "Velho Oeste",
            descricao = "Rua de terra entre mesas de arenito, ao pôr do sol.",
            gratis = false,
            productId = "cenario_velhooeste",
        ),
        Cenario(
            id = "heroi",
            nome = "Descanso do Herói",
            descricao = "Túmulo sob a árvore, com o moinho e a vila ao longe.",
            gratis = false,
            productId = "cenario_heroi",
        ),
        Cenario(
            id = "versalhes",
            nome = "Versalhes",
            descricao = "Espelho d'água e o palácio ao fundo.",
            gratis = false,
            productId = "cenario_versalhes",
        ),
        Cenario(
            id = "babilonia",
            nome = "Jardins da Babilônia",
            descricao = "Zigurate em terraços sobre o rio.",
            gratis = false,
            productId = "cenario_babilonia",
        ),
        Cenario(
            id = "telhados",
            nome = "Telhados",
            descricao = "Terraço sobre os telhados da cidade velha.",
            gratis = false,
            productId = "cenario_telhados",
        ),
        Cenario(
            id = "forte",
            nome = "Forte na água",
            descricao = "Fortaleza cercada de água.",
            gratis = false,
            productId = "cenario_forte",
        ),
        Cenario(
            id = "camboja",
            nome = "Templo das raízes",
            descricao = "Ruína khmer engolida pelas raízes.",
            gratis = false,
            productId = "cenario_camboja",
        ),
        Cenario(
            id = "caverna",
            nome = "Boca da caverna",
            descricao = "Boca de caverna sobre o açude.",
            gratis = false,
            productId = "cenario_caverna",
        ),
        Cenario(
            id = "estacionamento",
            nome = "Mirante de asfalto",
            descricao = "Mirante de asfalto sobre a baía.",
            gratis = false,
            productId = "cenario_estacionamento",
        ),
        Cenario(
            id = "mureta",
            nome = "Muro do campo",
            descricao = "Muro de pedra e o vale aberto.",
            gratis = false,
            productId = "cenario_mureta",
        ),
        Cenario(
            id = "porto",
            nome = "Porto medieval",
            descricao = "Cais medieval com o navio atracado.",
            gratis = false,
            productId = "cenario_porto",
        ),
        Cenario(
            id = "terrao",
            nome = "Campinho de várzea",
            descricao = "Campinho de terra na beira da vila.",
            gratis = false,
            productId = "cenario_terrao",
        ),
        Cenario(
            id = "bruxa",
            nome = "Casa da bruxa",
            descricao = "Casario torto de bruxa à beira do lago.",
            gratis = false,
            productId = "cenario_bruxa",
        ),
        Cenario(
            id = "eiffel",
            nome = "Torre Eiffel",
            descricao = "Campo de Marte com a torre ao fundo.",
            gratis = false,
            productId = "cenario_eiffel",
        ),
        Cenario(
            id = "simpsons",
            nome = "Casa amarela",
            descricao = "Rua de subúrbio com a casa amarela.",
            gratis = false,
            productId = "cenario_casaamarela",
        ),
        Cenario(
            id = "budokai",
            nome = "Arena de torneio",
            descricao = "Ringue de artes marciais entre montanhas.",
            gratis = false,
            productId = "cenario_arena",
        ),
        Cenario(
            id = "konoha",
            nome = "Vila ninja",
            descricao = "Rua da vila entre os penhascos, sob a montanha.",
            gratis = false,
            productId = "cenario_vilaninja",
        ),
        Cenario(
            id = "trincheira",
            nome = "Trincheira",
            descricao = "Terra de ninguém vista da trincheira.",
            gratis = false,
            productId = "cenario_trincheira",
        ),
        Cenario(
            id = "cofre",
            nome = "Câmara do ouro",
            descricao = "Câmara-forte transbordando de ouro.",
            gratis = false,
            productId = "cenario_cofre",
        ),
        Cenario(
            id = "ruinas",
            nome = "Ruínas do tesouro",
            descricao = "Ruínas tomadas por um mar de moedas.",
            gratis = false,
            productId = "cenario_ruinas",
        ),
        Cenario(
            id = "baobas",
            nome = "Baobás",
            descricao = "Vale de baobás à beira do rio.",
            gratis = false,
            productId = "cenario_baobas",
        ),
        Cenario(
            id = "cafeparis",
            nome = "Café de Paris",
            descricao = "Esquina parisiense com o toldo listrado.",
            gratis = false,
            productId = "cenario_cafeparis",
        ),
        Cenario(
            id = "estrada",
            nome = "Estrada do deserto",
            descricao = "Reta de terra entre cactos e mesas.",
            gratis = false,
            productId = "cenario_estrada",
        ),
        Cenario(
            id = "praiaeuropa",
            nome = "Praia europeia",
            descricao = "Enseada mediterrânea com guarda-sóis.",
            gratis = false,
            productId = "cenario_praiaeuropa",
        ),
        Cenario(
            id = "cyberpunk",
            nome = "Cyberpunk — canal",
            descricao = "Passarelas de neon sobre o canal cheio.",
            gratis = false,
            productId = "cenario_cyberpunk",
        ),
        Cenario(
            id = "cyberseco",
            nome = "Cyberpunk — rua",
            descricao = "A mesma cidade pela rua seca, entre letreiros.",
            gratis = false,
            productId = "cenario_cyberseco",
        ),
        Cenario(
            id = "quarto",
            nome = "Quarto na chuva",
            descricao = "Quarto pequeno com a janela larga sobre os telhados.",
            gratis = false,
            productId = "cenario_quarto",
        ),
        Cenario(
            id = "cafeinterno",
            nome = "Café na esquina",
            descricao = "Mesa de mármore junto à vidraça da cafeteria.",
            gratis = false,
            productId = "cenario_cafeinterno",
        ),
        Cenario(
            id = "estufa",
            nome = "Estufa de vidro",
            descricao = "Vasos e samambaias sob o teto de vidro.",
            gratis = false,
            productId = "cenario_estufa",
        ),
        Cenario(
            id = "trem",
            nome = "Vagão de trem",
            descricao = "Bancos de couro e o campo passando na janela.",
            gratis = false,
            productId = "cenario_trem",
        ),
        Cenario(
            id = "orbita",
            nome = "Estação orbital",
            descricao = "Convés de observação sobre a curva do planeta.",
            gratis = false,
            productId = "cenario_orbita",
        ),
        Cenario(
            id = "biblioteca",
            nome = "Biblioteca antiga",
            descricao = "Estantes e a janela em arco sobre a mesa de leitura.",
            gratis = false,
            productId = "cenario_biblioteca",
        ),
        Cenario(
            id = "natalsala",
            nome = "Sala de Natal",
            descricao = "Lareira, árvore e a rua nevada pela janela.",
            gratis = false,
            productId = "cenario_natalsala",
        ),
        Cenario(
            id = "natalchale",
            nome = "Chalé de Natal",
            descricao = "Chalé de montanha no caminho de neve pisada.",
            gratis = false,
            productId = "cenario_natalchale",
        ),
        Cenario(
            id = "halloween",
            nome = "Casarão assombrado",
            descricao = "Casarão torto no alto da colina, entre abóboras.",
            gratis = false,
            productId = "cenario_halloween",
        ),
        Cenario(
            id = "nortada",
            nome = "Cabana da aurora",
            descricao = "Cabana isolada na margem do lago congelado.",
            gratis = false,
            productId = "cenario_nortada",
        ),
        Cenario(
            id = "cachoeira",
            nome = "Cachoeira da mata",
            descricao = "Queda de água vista da passarela de madeira.",
            gratis = false,
            productId = "cenario_cachoeira",
        ),
        Cenario(
            id = "aldeiajapao",
            nome = "Aldeia de inverno",
            descricao = "Rua de casas de madeira subindo até o templo.",
            gratis = false,
            productId = "cenario_aldeiajapao",
        ),
        Cenario(
            id = "natal",
            nome = "Vila de Natal",
            descricao = "Praça de vila europeia com o pinheiro enfeitado.",
            gratis = false,
            productId = "cenario_natal",
        ),
        Cenario(
            id = "vulcao",
            nome = "Vulcão da ilha",
            descricao = "Cone escuro sobre a praia de areia vulcânica.",
            gratis = false,
            productId = "cenario_vulcao",
        ),
        Cenario(
            id = "enseada",
            nome = "Lago na neblina",
            descricao = "Píer de tábuas e a ilhota de uma árvore só.",
            gratis = false,
            productId = "cenario_enseada",
        ),
        Cenario(
            id = "lavanda",
            nome = "Campo de lavanda",
            descricao = "Fileiras de lavanda subindo até a casa de pedra.",
            gratis = false,
            productId = "cenario_lavanda",
        ),
        Cenario(
            id = "mesquita",
            nome = "Pátio da mesquita",
            descricao = "Pátio de mármore visto da arcada.",
            gratis = false,
            productId = "cenario_mesquita",
        ),
        Cenario(
            id = "himalaia",
            nome = "Mosteiro da montanha",
            descricao = "Mosteiro agarrado ao esporão de rocha.",
            gratis = false,
            productId = "cenario_himalaia",
        ),
        Cenario(
            id = "recife",
            nome = "Naufrágio no coral",
            descricao = "Casco afundado entre os corais.",
            gratis = false,
            productId = "cenario_recife",
        ),
        Cenario(
            id = "ogro",
            nome = "Casa no tronco",
            descricao = "Casa cavada no toco de uma árvore gigante.",
            gratis = false,
            productId = "cenario_ogro",
        ),
        Cenario(
            id = "rio",
            nome = "Mirante da baía",
            descricao = "Parapeito de pedra sobre a baía e os morros.",
            gratis = false,
            productId = "cenario_rio",
        ),
        Cenario(
            id = "torii",
            nome = "Torii no lago",
            descricao = "Portal vermelho erguido na água rasa.",
            gratis = false,
            productId = "cenario_torii",
        ),
        Cenario(
            id = "vigia",
            nome = "Torre de vigia",
            descricao = "Torre de pedra tomada por hera na ponta do penhasco.",
            gratis = false,
            productId = "cenario_vigia",
        ),
        Cenario(
            id = "esfinge",
            nome = "Esfinge",
            descricao = "Esfinge e pirâmides ao fim do dia.",
            gratis = false,
            productId = "cenario_esfinge",
        ),
        Cenario(
            id = "sitio",
            nome = "Celeiro vermelho",
            descricao = "Celeiro vermelho à beira do lago.",
            gratis = false,
            productId = "cenario_sitio",
        ),
        Cenario(
            id = "savana",
            nome = "Savana",
            descricao = "Acácia e poço d'água na planície.",
            gratis = false,
            productId = "cenario_savana",
        ),
        Cenario(
            id = "oasis",
            nome = "Oásis",
            descricao = "Cidade de pedra ao redor do lago no deserto.",
            gratis = false,
            productId = "cenario_oasis",
        ),
        Cenario(
            id = "elfico",
            nome = "Refúgio élfico",
            descricao = "Casa nas raízes da árvore antiga.",
            gratis = false,
            productId = "cenario_elfico",
        ),
        Cenario(
            id = "postapoc",
            nome = "Cidade tomada",
            descricao = "Metrópole em ruínas, engolida pela mata.",
            gratis = false,
            productId = "cenario_postapoc",
        ),
        Cenario(
            id = "navio",
            nome = "Navio viking",
            descricao = "Drakkar a remo em mar aberto.",
            gratis = false,
            productId = "cenario_navio",
        ),
        Cenario(
            id = "castelo",
            nome = "Castelo na Montanha",
            descricao = "Castelo no pico, acima de um mar de nuvens.",
            gratis = false,
            productId = "cenario_castelo",
        ),
        Cenario(
            id = "beco",
            nome = "Beco japonês",
            descricao = "Ruela de izakayas, lanternas e cerejeira.",
            gratis = false,
            productId = "cenario_beco",
        ),
        Cenario(
            id = "templo",
            nome = "Templo grego",
            descricao = "Templo no penhasco, de frente para o Egeu.",
            gratis = false,
            productId = "cenario_templo",
        ),
        Cenario(
            id = "vale",
            nome = "Vale alpino",
            descricao = "Rio entre montanhas, vila e roda d'água.",
            gratis = false,
            productId = "cenario_vale",
        ),
        Cenario(
            id = "muralha",
            nome = "Muralha da China",
            descricao = "A muralha serpenteando entre os picos, com o rio ao fundo.",
            gratis = false,
            productId = "cenario_muralha",
        ),
        Cenario(
            id = "jardim",
            nome = "Jardim japonês",
            descricao = "Cerejeiras, ponte vermelha e lago ao pé do monte.",
            gratis = false,
            productId = "cenario_jardim",
        ),
        Cenario(
            id = "pantano",
            nome = "Pântano de névoa",
            descricao = "Água parada, cipós e névoa entre as árvores.",
            gratis = false,
            productId = "cenario_pantano",
        ),
        Cenario(
            id = "farol",
            nome = "Farol",
            descricao = "Farol no penhasco, de frente para o mar aberto.",
            gratis = false,
            productId = "cenario_farol",
        ),
        Cenario(
            id = "praia",
            nome = "Praia tropical",
            descricao = "Enseada de areia clara, coqueiros e um galeão no horizonte.",
            gratis = false,
            productId = "cenario_praia",
        ),
        Cenario(
            id = "duomo",
            nome = "Duomo",
            descricao = "Catedral de Milão vista da praça de pedra.",
            gratis = false,
            productId = "cenario_duomo",
        ),
        Cenario(
            id = "coliseu",
            nome = "Coliseu",
            descricao = "O anfiteatro de Roma sobre o calçamento antigo.",
            gratis = false,
            productId = "cenario_coliseu",
        ),
        Cenario(
            id = "notredame",
            nome = "Notre Dame",
            descricao = "A catedral de Paris no fim do parvis.",
            gratis = false,
            productId = "cenario_notredame",
        ),
        Cenario(
            id = "cidadeproibida",
            nome = "Cidade Proibida",
            descricao = "Pátio imperial e o telhado dourado ao fundo.",
            gratis = false,
            productId = "cenario_cidadeproibida",
        ),
        Cenario(
            id = "burj",
            nome = "Burj Khalifa",
            descricao = "A torre mais alta do mundo sobre os espelhos d’água.",
            gratis = false,
            productId = "cenario_burj",
        ),
        Cenario(
            id = "timessquare",
            nome = "Times Square",
            descricao = "A avenida de painéis luminosos, vazia na chuva.",
            gratis = false,
            productId = "cenario_timessquare",
        ),
        Cenario(
            id = "pizzaria",
            nome = "Mesa no beco italiano",
            descricao = "Mesa na calçada de um beco italiano.",
            gratis = false,
            productId = "cenario_pizzaria",
        ),
        Cenario(
            id = "vilaviking",
            nome = "Vila viking",
            descricao = "Casas de telhado de grama à beira do fiorde.",
            gratis = false,
            productId = "cenario_vilaviking",
        ),
        Cenario(
            id = "apefino",
            nome = "Sala com vista",
            descricao = "Sala envidraçada de frente para o parque.",
            gratis = false,
            productId = "cenario_apefino",
        ),
    )

    fun por(id: String): Cenario? = cenarios.firstOrNull { it.id == id }
    val padrao: Cenario get() = cenarios.first()
}
